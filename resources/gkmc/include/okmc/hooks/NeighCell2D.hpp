...
#ifndef GKMC_NeighCell2D_H
#define GKMC_NeighCell2D_H
...
template <class SysProps> class NeighCell2D {
  static_assert(SysProps::dims == 2);
...
public:
  NeighCell2D(const Coords &boxDimensions, const value_type &cellLength,
            const bool &isMirror)
  ...
  void added(Sys &, const Particle &p, const Pid &pid) {
    std::array<int, dims> pos;
    const auto &c = p.coords();
    const Species &species = pid.first; // TODO
    for (auto i = 0; i < dims; ++i)
      pos[i] = c[i] / _cellLength;
    if (pos[0] < _nCells[0] && pos[1] < _nCells[1] &&
        pos[0] >= 0 && pos[1] >= 0) {
      _cells[pos[0]][pos[1]][species].insert(pid.second);
    }
  }

  template <class Sys, class Particle>
  void removed(Sys &sys, const Particle &p, const Pid &pid) {

  }
  template <class Sys, class Particle>
  void editedCoords(Sys &, const Particle &p, const Pid &pid, const Coords &pre) {
    ...
    if (flag) {
      if (posPre[0] < _nCells[0] && posPre[1] < _nCells[1] &&
          posPre[0] >= 0 && posPre[1] >= 0)
        _cells[posPre[0]][posPre[1]][sp].erase(index);
      if (posCur[0] < _nCells[0] && posCur[1] < _nCells[1] &&
          posCur[0] >= 0 && posCur[1] >= 0)
        _cells[posCur[0]][posCur[1]][sp].insert(index);
    }
  }
  template <class Sys, class Particle>
  void edited(Sys &, const Particle &p, const Pid &pid) {}

  template <class Sys, class Particle>
  void erased(Sys& sys, const Particle &p, const Pid &pid) {
    ...
    if (pos[0] < _nCells[0] && pos[1] < _nCells[1] &&
        pos[0] >= 0 && pos[1] >= 0) {
      _cells[pos[0]][pos[1]][species].erase(index);
    }
    ...
    if (n && index != (n - 1)) {
      ...
      if (pos[0] < _nCells[0] && pos[1] < _nCells[1] &&
          pos[0] >= 0 && pos[1] >= 0) {
        _cells[pos[0]][pos[1]][species].erase(n - 1);
        _cells[pos[0]][pos[1]][species].insert(index);
      }
    }
  }
  ...
  auto neighbour(Sys &sys, const typename Sys::Pid &pid,
    ...
    for (auto i = -1; i < 2; ++i) {
      ...
      for (auto j = -1; j < 2; ++j) {
        ...
        if (cj < 0)
          cj += _nCells[1];
        for (auto it : _cells[ci][cj]) {
          if (sp != it.first) continue;
