...
public:
  ...
  auto neighbour(const Sys &sys, const typename Sys::Pid &pid,
        ...
        for (auto k = -1; k < 2; ++k) {
          ...
          for (auto it : _cells[ci][cj][ck]) {
            ...
            for (auto jt : it.second) {
              ...
              const auto &jPart = sys.template particle<P2>(sys.cookPid(it.first, jt));
              if (pivot.id() == jPart.id()) continue;
              tol = capRadius(pivot.size(), jPart.size());
              if (pred(c, jPart.coords(), tol))
                return std::make_pair(true, sys.cookPid(it.first, jt));
            }
          }
        }
      }
