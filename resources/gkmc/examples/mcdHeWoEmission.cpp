...
class SpawnBySteps : public Dummy {
  int _step = 0;
  int _interval;
  SpawnSingle<P, Species> _spawn{0.0, Species::He};
public:
  SpawnBySteps(int interval) : _interval{interval} {}
  template <class System>
  auto update(System &sys, double t, double dt) {
    if (++_step == 30303) {
      _step = 0;
      _spawn.execute(sys);
    } 
...
template <class Sys>
auto processes(double w) {
  return Diffuse3D<P, Species>{Species::He, probInvars::jumpLen * std::sqrt(w)};
}
...
void testDiffCoeff(double w, std::string fname) {
  ...
  auto spn = SpawnBySteps{30303};
  auto hookSpawn = both(hook, spn);
  auto sys = system(props, hookSpawn);
  auto procs = processes<decltype(sys)>(w);
  Logger<decltype(sys)> oo(fname);
  auto pred = TimePredicate<decltype(sys), decltype(oo)&>{probInvars::maxTime, oo};
  auto delTFromPokmc = 1.5e-6;
  auto sysTime = mcd(procs, pred, sys, delTFromPokmc * w);
