...
auto run(double temperature) {
  ...
  // problem constants
  constexpr double ws[] = {1.717e15, 1.1282e15};
  constexpr double ems[] = {0.5, 0.6};
  constexpr int counts[] = {100, 20};
  constexpr double initRadius[] = {60.0, 20.0};
  const auto rCalc = [](const auto &, const auto &, const auto &, const auto &, const auto &) { return 4.0; };
  constexpr double jump = 2.855*0.8662;
  ...
  auto reaction = Reaction{Annihilation{}, std::vector<Species>{Species::Fe}, std::vector<Species>{Species::Cr}, NeighBasic{false}, rCalc};
  auto hook = Hook{reaction};
  auto sys = Sys{props, hook};
  // adding process with wrapper for counting
  Procs procs;
  procs.add(ProcCounter{ProcWrap{Proc{Species::Fe, ems[0], ws[0], jump, temperature}}});
  procs.add(ProcCounter{ProcWrap{Proc{Species::Cr, ems[1], ws[1], jump, temperature}}});
  // adding SystemTuple
  /*
  ...
  */
  ...
  auto printSystemTupleCount = [&steps, dumpInterval](Sys &p, double time) {
    if (!steps || steps % dumpInterval)
      return;
    auto ps = p.template particles<Particle>(Species::Fe); // TODO: for V
    for (auto it : ps) {
      std::cout << time << "\t ";
